...
Before returning the final rewrite, scan for `—` and `–` and resolve every hit per the §14 three-case rule above. The Vale pre-pass surfaces these via `local.EmDashUsage` at `warning` level.
...
When you need a complete before/after rewrite example (original AI-sounding text, draft rewrite, audit, and final version), read references/full-example.md.
